...
ALTER PROCEDURE spREP_SECTIONLIST
WITH ENCRYPTION
AS
BEGIN
SELECT RTS.id, RTS.SY, RTS.levelCode, RTS.sectionCode, UPPER(RTS.levelCode + '-' + RTS.sectionCode) as levelSection,RTS.roomID,
	RTS.teacherID, UPPER(RTS.schedDesc) as Schedule, UPPER(URL.roomDescription)as roomDescription, UPPER(RTS.bldgDesc) as buildingDesc,
	 USL.sectionDesc,UPPER(XUC.Uname)as TeacherName, RSM.StudNum, 
	(CASE WHEN RSI.ReligionCode = 'CAT' THEN RSM.StudName 
		  ELSE RSM.StudName + ' *' END) as StudendName, RSI.ReligionCode, RSI.GenderCode, XLT.LevelTypeDesc
	...
	ON RTS.levelCode = XLT.LevelTypeCode
